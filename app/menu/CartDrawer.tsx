  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      setOrderSuccess({
        ...
        table_session_id: res.table_session_id,
      ...
      >
        ...
        {orderSuccess ? (
          <div className="flex flex-1 flex-col items-center justify-center px-6 py-10 text-center">
            ...
            {orderSuccess.table_session_id && (
              <p className="mt-2 text-xs text-gray-500">
                Check session: {orderSuccess.table_session_id}
              </p>
            )}
            {orderSuccess.display_total != null && selectedCurrency && (
              <p className="mt-2 text-base font-bold text-gray-900">
                {selectedCurrency.symbol}{" "}
                {orderSuccess.display_total.toFixed(2)}
              </p>
            )}
            <button
              type="button"
                  ...
                  <ul className="space-y-2.5">
                    {entries.map((entry, index) => {
                      ...
                      const modTotal = entry.selectedModifiers.reduce(
                        (s, m) =>
                          s +
                          (m.modifier.display_price ?? m.modifier.price) *
                            m.quantity,
                        0,
                      );
